...
export default function TV() {
  ...
  return (
    <>
      {DataTV ?       <div className="container mt-4 ">
        ...
        <Slider {...settings}>
          {DataTV.map((item, index) => (
            <img key={index} src={`https://image.tmdb.org/t/p/w500/${item.poster_path}`} className="w-75 rounded-3" alt={item.original_title} />
          ))}
        </Slider>
        <div className="row">
          ...
          {DataTV.filter((item) =>"https://image.tmdb.org/t/p/w500/" + item.poster_path !== null).map((item, index) => (
            <div className="col-lg-3 col-md-4 col-sm-6 p-2" key={index}>
              <div className="card h-100">
                <div className='overflow-hidden'>
                  <img src={`https://image.tmdb.org/t/p/w500/${item.poster_path}`} className="card-img-top w-100" alt={item.original_title} />
                </div>
      ...
      </div> : <div className='d-flex justify-content-center align-items-center vh-100 text-white'>
